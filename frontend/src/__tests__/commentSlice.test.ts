import reducer, { fetchCommentsData, addNewComment } from '../features/commentSlice'
...
describe('fetchCommentsData', () => {
  it('Should not work fetchCommentsData when pending', () => {
    const action = { type: fetchCommentsData.pending.type, payload: { commentData } }
    const state = reducer(initialState, action)
    expect(state.comments).toEqual([])
    expect(state.status).toEqual('loading')
  })
  it('Should work fetchCommentsData when fulfilled', () => {
    const action = { type: fetchCommentsData.fulfilled.type, payload: { commentData } }
    const state = reducer(initialState, action)
...
describe('addNewComment', () => {
  it('Should work addNewComment when fulfilled', () => {
    const action = { type: addNewComment.fulfilled.type, payload: { commentData } }
    const state = reducer(initialState, action)
